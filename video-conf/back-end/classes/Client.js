...
class Client {
  ...
  addTransport(type, audioPid = null, videoPid = null) {
    // 6.2 Reuse createWebRtcTransportBothKinds (ugh!) from theBasics project
    return new Promise(async (resolve, reject) => {
      const { listenIps, initialAvailableOutgoingBitrate, maxIncomingBitrate } =
        config.webRtcTransport; // 6.3 Add a few new details into it
      ...
      // 6.4 Add new transport to the right part of our Client object
      if (type === "producer") {
      ...
      resolve(clientTransportParams); // 6.5 Respond with our clientTransportParams
    });
  }
